...
class TMobileTAAPClient:
    ...
    async def subscriber_inquiry(self, msisdn: str) -> dict[str, Any]:
        ...
        activation and returned asynchronously via the callback — so until one
        exists this is intentionally disabled (kept secondary to activation).
        Set ``TMOBILE_ACCOUNT_ID`` once the activation callback yields it.
        """
    ...
    async def activate_subscriber(
        ...
        **extra: Any,
        ...
        """Activate a new subscriber line (IoT / Infatrac).
        ...
        asynchronously to the ``call-back-location``.  The request carries
        ``marketZIP`` + ``ICCID`` + ``productId``.

        Inputs fall back to env when not passed explicitly:
        ``market_zip`` -> TMOBILE_MARKET_ZIP, ``product_id`` -> TMOBILE_PRODUCT_ID,
        ``callback_location`` -> TMOBILE_CALLBACK_LOCATION.  The partner /
        ...
        A ``call-back-location`` is REQUIRED — the generated account ID is
        returned only to that callback, so this raises ``ValueError`` rather
        than send an activation we could never reconcile.
        """
        ...
        # Fail closed: the account ID is GENERATED by activation and returned
        # ONLY to this callback.  A live activation without call-back-location
        # would succeed at T-Mobile but strand the generated account ID with no
        # way for us to capture it — so require it rather than silently omit.
        ...
        **extra: Any,
        ...
        if not market_zip:
            raise ValueError(
                ...
                "TMOBILE_MARKET_ZIP, e.g. 30338/30346 for PIT)"
            )
        if not product_id:
            raise ValueError(
                "activate_subscriber requires a productId (pass product_id or set "
                "TMOBILE_PRODUCT_ID — pending T-Mobile)"
            )
