...
from app.integrations import tmobile_operations as OPS  # noqa: E402
from app.integrations.tmobile_operations import (  # noqa: E402
    operation_for_request,
    require_live_sendable,
)


def _subscriber_selector(
    *, msisdn: str | None, iccid: str | None, imsi: str | None
) -> dict[str, str]:
    """Build the identifier body shared by the subscriber query operations.

    Exactly one of the three is enough, and sending more than we were given
    would be inventing input, so only the supplied values travel. Raising here
    keeps a malformed request from reaching OAuth.
    """
    body = {k: v.strip() for k, v in
            (("msisdn", msisdn), ("iccid", iccid), ("imsi", imsi)) if v and v.strip()}
    if not body:
        raise ValueError(
...
class TMobileTAAPClient:
    ...
    async def _request(
        ...
        *,
        ...
        """
        # ── Client boundary (fail closed) ──────────────────────────────
        # Runs BEFORE get_access_token() so a blocked operation costs zero
        # network calls rather than a failed one. This is deliberately here and
        # not only in the operator CLI: calling a client method directly must
        # not be a way around the certification gates.
        operation = operation_for_request(method, "/" + path.lstrip("/"))
    ...
    async def subscriber_inquiry(
        ...
        *,
        ...
        """Query subscriber detail. Any ONE of msisdn / iccid / imsi is enough.

        The earlier implementation required an account id and refused to run
        without one. That requirement was never part of the vendor contract —
        this operation identifies the subscriber by msisdn, iccid, or imsi, and
        takes no account id at all. The parameter is retained only so existing
        callers keep working; it is not transmitted.
        """
        body = _subscriber_selector(msisdn=msisdn, iccid=iccid, imsi=imsi)
        return await self._request(
            "POST", OPS.get_operation("subscriber_inquiry").path,
            json_body=body,
    ...
    async def query_network(
        ...
        *,
        ...
        """Query how a subscriber is provisioned on the network."""
        body = _subscriber_selector(msisdn=msisdn, iccid=iccid, imsi=imsi)
        return await self._request(
            "POST", OPS.get_operation("query_network").path,
            json_body=body,
            extra_headers=self._callback_headers(callback_location),
        )
    ...
    async def query_usage(
        ...
        *,
        ...
        """Query voice/messaging/data/wallet usage for a subscriber.

        Takes no date range: the earlier signature required start and end dates,
        which are not fields of this operation.
        """
        body = _subscriber_selector(msisdn=msisdn, iccid=iccid, imsi=imsi)
        return await self._request(
            "POST", OPS.get_operation("query_usage").path, json_body=body)
    ...
    async def query_transaction_status(self, transaction_id: str) -> dict[str, Any]:
        ...
        This is the supported way to investigate a provisioning request whose
        asynchronous result has not arrived — instead of resending the request,
        which risks a duplicate provisioning action. It is an on-need
        operational-support call: never polled, scheduled, or run in bulk.
        """
        if not (transaction_id or "").strip():
            raise ValueError("query_transaction_status requires a transaction id.")
        return await self._request(
            "POST", OPS.get_operation("query_transaction_status").path,
            json_body={"transactionId": transaction_id.strip()},
        )
    ...
    async def change_sim(
        ...
        *,
        ...
        """Swap a subscriber's SIM. DESTRUCTIVE — the replaced SIM ages out.

        ``current_iccid`` is the SIM being replaced and ``new_iccid`` is its
        replacement. The earlier implementation sent the replacement in the
        field reserved for the current SIM, which would have swapped the wrong
        direction, and it never sent the replacement field at all.
        """
        body: dict[str, Any] = {
            "msisdn": msisdn, "iccid": current_iccid, "newIccid": new_iccid,
        }
        if pairing_id:
            body["pairingId"] = pairing_id
        return await self._request(
            "PUT", OPS.get_operation("change_sim").path, json_body=body,
            extra_headers=self._callback_headers(callback_location),
        )
    ...
    async def suspend_subscriber(
        self, msisdn: str, *, iccid: str, pairing_id: str | None = None
    ) -> dict[str, Any]:
        """Suspend an active line. Reversible via restore_subscriber.

        Both msisdn and iccid are required by the contract; the earlier
        implementation sent an account id instead of the iccid.
        """
        body: dict[str, Any] = {"msisdn": msisdn, "iccid": iccid}
        if pairing_id:
            body["pairingId"] = pairing_id
        return await self._request(
            "PUT", OPS.get_operation("suspend_subscriber").path, json_body=body)

    async def restore_subscriber(
        self, msisdn: str, *, iccid: str, pairing_id: str | None = None
    ) -> dict[str, Any]:
        """Restore a suspended line to active with its previous products."""
        body: dict[str, Any] = {"msisdn": msisdn, "iccid": iccid}
        if pairing_id:
            body["pairingId"] = pairing_id
        return await self._request(
            "PUT", OPS.get_operation("restore_subscriber").path, json_body=body)

    async def deactivate_subscriber(
        self, msisdn: str, *, iccid: str, pairing_id: str | None = None
    ) -> dict[str, Any]:
        """Deactivate a line. DESTRUCTIVE — treated as terminal here.

        A reactivation operation exists in the vendor contract but is neither
        implemented nor tested here, and is not assumed to restore the same
        number or plans.
        """
        body: dict[str, Any] = {"msisdn": msisdn, "iccid": iccid}
        if pairing_id:
            body["pairingId"] = pairing_id
        return await self._request(
            "PUT", OPS.get_operation("deactivate_subscriber").path, json_body=body)
