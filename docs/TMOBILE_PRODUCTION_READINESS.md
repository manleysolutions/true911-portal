...
| 12g | **Read-only certification tooling (4 ops)** | 🟡 *new* | Preview-by-default commands and per-operation single-run authorization for all four read-only operations. **Zero executed** — blocked on operator inputs. Persistence and the internal view deliberately deferred until a real response exists. |
| 12f | **Typed callback rules in shadow** | 🟡 *new* | Wired into the deployed processor behind `FEATURE_TMOBILE_CALLBACK_TYPED_SHADOW` (default off), observation only. Cannot become authoritative until lifecycle transactions are persisted — until then the correlation set is empty and every callback would quarantine. |
| 12e | **Single-run PIT authorization** | ✅ *new* | One read-only operation, one nominated subscriber, one request, PIT-only, time-boxed, consumed on use, auditable. Cannot cover a mutation. Does not weaken the global guard — a missing or mismatched grant falls through to the normal refusal. |
| 12d | **Typed contracts + lifecycle model** | ✅ *new 2026-07-21* | Typed request/response models validating before OAuth; normalized lifecycle with sync-acceptance separated from async completion; exact-correlation callback application with quarantine; carrier snapshot. Durable persistence deferred — migration chain is branched. |
| 12c | **Contract reconciliation** | ✅ *new 2026-07-21* | Implementation reconciled against authorized vendor documentation (`TMO-REST-RECON-001`): seven wrong paths, four wrong HTTP methods, and every lifecycle request body corrected. All eight non-activation operations remain live-blocked — documentation does not authorize sending. See `TMOBILE_OPERATION_READINESS.md`. |
...
| 15 | **Deactivation / suspension safeguards** | ✅ *closed 2026-07-21* | The client methods themselves are still ungated — calling `deactivate_subscriber` directly bypasses everything. But they are now **unreachable through the sanctioned path**: `scripts/tmobile_pit.py` blocks them outright (undocumented paths) and, even once documented, requires `--confirm-live` + `--confirm-destructive` + `--reason` + destructive-tier allowlisting. **CLOSED:** a fail-closed guard now runs inside the client itself, before the OAuth token is fetched, so a direct `client.deactivate_subscriber(...)` call raises without making a single network request. The safety no longer depends on which entry point an operator picks. |
